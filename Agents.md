...
| `MainPage.xaml.cs` | Edit carefully | `NavigationCoordinator` | Code-behind for `MainPage`. **Contains no business logic.** Responsibilities: subscribe to `MainViewModel.RefreshRequested` in constructor; implement `OnMenuClicked`, `OnTimelineHeadingTapped`, `OnLogoTapped` (opens `TeasePopup` anchored below NavBar, left-aligned, with Copy-to-clipboard and Close buttons); implement 11 `OnXxxInfoClicked` handlers that push `DeepDivePopup`; implement `OnTickerRefreshRequested` that pushes `RefreshingPopup`. Guard flags (`_isXxxOpen`) on every push prevent double-open. `OpenDeepDiveAsync()` measures `NavBar.Height + TimelineHeading.Height` to pass as `topOffset` to `DeepDivePopup`. Holds 15 guard bools (14 deep-dive/popup guards + `_isTeasePopupOpen`). Overrides `OnAppearing`/`OnDisappearing` to start and abort the `"LiveBadgeBreathing"` `Animation` that pulses all 10 LIVE badge labels (opacity 1.0 -> 0.4 -> 1.0, 2500 ms, `Easing.SinInOut`, repeating). `OnAppearing` also starts the Ambient Sparks loop (`StartAmbientSparks`) if `VibrantCosmosExpanded` is true; `OnDisappearing` stops it. `OnViewModelPropertyChanged` starts or stops the loop on `VibrantCosmosExpanded` changes. `ApplyBirthRankChart` sets `BirthRankChart.Drawable` to a fresh `BirthRankChartDrawable` and calls `Invalidate()` when `HumanBirthRankExpanded` or `HumanBirthRank` changes; clears Drawable for pre-1900 dates. `ApplyWyrdWeb` rebuilds the Web of Wyrd Explorer: populates `WyrdRuneGrid` with 24 `Border`+`Label` tap-targets, sets `WyrdWebView.Drawable` to a new `WyrdWebDrawable`, updates `WyrdRuneName`/`WyrdRuneMeaning`; called on `BirthRuneExpanded`, `BirthRune`, `ColorScheme`, `DisplayLanguage` changes and from constructor. `OnWyrdRuneTapped` updates `_wyrdSelectedIndex`, re-highlights grid borders, updates labels, re-invalidates canvas; no ViewModel mutation. `_wyrdCatalogue`/`_wyrdSelectedIndex` are private UI-state fields. `ApplyEnneagram` sets `EnneagramView.Drawable` to a fresh `EnneagramDrawable(personalYearNumber)` and calls `Invalidate()`; called on `PersonalYearExpanded`, `PersonalYear`, `ColorScheme` changes and from constructor. `ApplyVolumeCubeAsync` (async Task): selects landmark from `_landmarks` table (21 entries, SizeM < cube edge, fallback Human), loads PNG via FileSystem.OpenAppPackageFileAsync into MemoryStream, sets LandmarkImage.Source+HeightRequest and LandmarkLabel FormattedString; builds VolumeCubeDescLabel FormattedString; triggered on YourBreathExpanded/YourBreath/ColorScheme/UseMetric/DisplayLanguage and from constructor + VolumeCubeView.SizeChanged. `ApplyCarbonBudgetChart` sets CarbonBudgetTitleLabel, CarbonBudgetDepletionLabel (string.Format Depletion, year), axis labels via SetDynamicResource, creates CarbonBudgetChartDrawable; called on GlobalExhaleExpanded/GlobalExhale/DisplayLanguage changes and from constructor; hidden pre-1900. `ApplyVibrantHumanityBars` builds two proportional stacked-bar `Grid`s inside `VibrantHumanityBarsContainer`: Arrivals bar (fills full width, 2 columns - Singletons/Twins) and Departures bar (WidthRequest set proportionally via `SizeChanged` at `totalDeaths/totalBirths` ratio, 3 columns - Heart/Cancer/Other); `BuildBar` helper creates rounded `Border` with `*`-star column `Grid`; `BuildLegend` helper builds `FlexLayout` swatches; `GetDynColor` reads current theme colour from resource dict at call time; called on `VibrantHumanityExpanded`, `VibrantHumanity`, `ColorScheme`, `DisplayLanguage` changes and from constructor. `ApplyLifeLogChart` creates a `LifeLogChartDrawable` from `LifeLog.ActivitySlices`, assigns to `LifeLogChartView.Drawable`, clears and rebuilds `LifeLogLegend` children (7 Grid rows: colour swatch BoxView + name Label + YearsToday Label + YearsForecast Label), calls `Invalidate()`; called on `LifeLogExpanded`, `LifeLog` changes and from constructor. `ApplyPopulationChart` creates a `PopulationChartDrawable` from `GlobalCrowdResult` fields (BaseYear/CurrentYear/populations), stores it in `_populationChartDrawable`, initialises `HoverYear`/`HoverPopulation` to current year, assigns to `PopulationChartView.Drawable`; called on `GlobalCrowdExpanded`, `GlobalCrowd`, `ColorScheme` changes and from constructor. `OnPopulationChartInteraction` (StartInteraction/DragInteraction) clamps touch X to chart area, updates `_populationChartDrawable.ScrubX`, reverse-maps X to year via `PopulationChartDrawable.InterpolatePopulation`, writes `GlobalCrowd.HoverYear`/`HoverPopulation`, calls `Invalidate()`. `OnPopulationChartEndInteraction` snaps scrubber back to current year. `StartAmbientSparks` / `StopAmbientSparks` / `SpawnAmbientSparks` / `RunStarLifecycle` / `RunSupernovaOnLabel` implement the particle canvas: star-birth Labels (U+2736, 90%) are spawned at random positions and run independent `RunStarLifecycle` Tasks (fade in, random 1-10 s dwell, fade out); multiple stars are visible simultaneously; live stars tracked in `_liveStars` (lock-guarded List). Supernova (10%) picks a random entry from `_liveStars`, removes it, and runs `RunSupernovaOnLabel` on that existing Label (U+2739 swell then U+1F4A5 flash then dissipate) - no new position needed. Both U+2736 and U+2739 read `JubileeAccent` from `Application.Current.Resources` at spawn time (gold DefaultDark, white HC-Dark, black HC-Light). `_sparksRunning` guard flag prevents double-start. |
...
| `Views/MainPage.xaml` | ~1134 | Photon Path proportional track `Grid` - Row 0: NextStopText label; Row 1: `AbsoluteLayout` with full-width grey track `BoxView`, filled cyan track `BoxView` (x:Name PhotonTrackFill, width set imperatively), Sun dot `Ellipse`, star endpoint ring `Ellipse`, ship marker `Ellipse` (x:Name PhotonShipMarker, X set imperatively in MainPage.xaml.cs via ApplyPhotonTrackPosition); Row 2: Sun and NextStarName labels. All colours use DynamicResource. `IsVisible` gated on PhotonPathExpanded. |
| `Views/MainPage.xaml` | ~1055 | Galactic Commute Milky Way animation `Image` - `Source` is a XAML literal (`anim_sun_in_milky_way.gif`, not a binding) to prevent MAUI GIF decoder restart on each 1-second timer tick; `IsAnimationPlaying=True`; `IsVisible` bound to `GalacticCommuteExpanded`; drawn below the `FullText` label as a sibling in the `VerticalStackLayout` (same flat pattern as Countdown and Cellular Refresh). |
| `Views/MainPage.xaml` | ~1299 | Cosmic Stretch universe-expansion animation `Image` - `Source` is a XAML literal (`anim_cosmic_stretch.gif`, not a binding) to prevent MAUI GIF decoder restart on each 1-second timer tick; `IsAnimationPlaying=True`; `IsVisible` bound to `CosmicStretchExpanded`; drawn below the `FullText` label as a sibling in the `VerticalStackLayout` (same flat pattern as Countdown and Galactic Commute). |
| `Views/MainPage.xaml` | ~2143 | Your Breath volumetric cube - `VerticalStackLayout` x:Name=VolumeCubeContainer (IsVisible=YourBreathExpanded, Spacing=8): inner Grid (VolumeCubeView H=250 + LandmarkImage layered, ImageTint.Color=TextDim); below it a 2-col Grid with LandmarkLabel (col 0, start) and VolumeCubeDescLabel (col 1, end). |
| `Views/MainPage.xaml` | ~2028 | Global Exhale carbon budget chart - `VerticalStackLayout` x:Name=CarbonBudgetChartContainer (IsVisible=GlobalExhaleExpanded, Spacing=6): CarbonBudgetTitleLabel, CarbonBudgetDepletionLabel, CarbonBudgetChartView (H=220), Grid with CarbonChartLabelBase/Today/Limit. All text populated by ApplyCarbonBudgetChart(). |
| `Views/MainPage.xaml` | ~1934 | Vibrant Humanity balance bars - `VerticalStackLayout` x:Name=VibrantHumanityBarsContainer (self-closing, IsVisible=VibrantHumanityExpanded, Spacing=16); Arrivals and Departures bar stacks built imperatively by `ApplyVibrantHumanityBars`. |
| `Views/MainPage.xaml` | ~1846 | Life Log donut chart - `GraphicsView` x:Name=LifeLogChartView 260x260 (IsVisible=LifeLogExpanded); `VerticalStackLayout` x:Name=LifeLogLegend (7 rows built imperatively by `ApplyLifeLogChart`, IsVisible=LifeLogExpanded). |
| `Views/MainPage.xaml` | ~1737 | Global Crowd population chart - `VerticalStackLayout` (`IsVisible`=GlobalCrowdExpanded) with two `Label`s (HoverYear prefix + `FormattedString` value, HoverPopulation prefix + value) and `GraphicsView` x:Name=PopulationChartView (HeightRequest=180, FillAndExpand); StartInteraction/DragInteraction -> `OnPopulationChartInteraction`; EndInteraction -> `OnPopulationChartEndInteraction`. |
| `Views/MainPage.xaml` | ~1659 | Personal Year Enneagram - `GraphicsView` x:Name=EnneagramView 250x250 inside PersonalYear card, `IsVisible`=PersonalYearExpanded; Drawable set imperatively by `ApplyEnneagram`. |
...
- Contains private/public helpers: `FindNearestJubilee(long diff)`, `ReduceToSingleDigit(int num)`, `HumanPopulationByDate(DateTime date)` (shared 3-epoch UN population model), `HumanBirthRankbyDate(DateTime date)` (shared 3-epoch PRB birth-rank model), and `TotalDeathsByDate(DateTime date)` (derived from the two shared models). These three shared helpers are used by `CalculateGlobalCrowd`, `CalculateHumanBirthRank`, and `CalculateVibrantHumanity` to ensure mathematical consistency.
...
- **Before committing any change that adds a ticker, section, or `ICommand`, audit
  the Live Counters table below** and update every stale value in `Agents.md` first.
  Run each `Select-String` command, compare the result against the current `Agents.md`
  value, and correct any mismatch with a targeted `Replace` script before committing.

  **Live Counters - must match the codebase after every ticker/command change:**

  | Counter | Location(s) in `Agents.md` | Verification command |
  |---------|---------------------------|----------------------|
  | Typed result property count | §2 `MainViewModel` row; §5 Node 4 Responsibilities + Owns | `Select-String ViewModels\MainViewModel.cs -Pattern "public .+Result\s+\w+"` |
  | Card expansion bool count | §5 Node 4 Responsibilities + Owns (total = sections + cards) | `Select-String ViewModels\MainViewModel.cs -Pattern "private bool _\w+Expanded"` |
  | `ICommand` count + breakdown | §5 Node 4 Responsibilities | `Select-String ViewModels\MainViewModel.cs -Pattern "^\s+public ICommand "` |
  | `CalculationService` method count | §5 Node 5 method registry table | `Select-String Services\CalculationService.cs -Pattern "public \w+Result Calculate\w+"` |
...
- **Do not run a `.ps1` script that targets `Agents.md`, `.resx` files, or
  `AppResources.Designer.cs` without reading it back first.** After `create_file`,
  immediately call `get_file` on the `.ps1` itself and verify:
  - Every `[System.Text.Encoding]::UTF8` cast uses `::` (double-colon, no angle brackets).
  - Every PowerShell string interpolation uses the correct quoting for the target shell.
  - The `Write-Host "done"` (or equivalent confirmation line) is present and will
    actually execute on the success path.
  If any of these checks fail, use `remove_file` + `create_file` to replace the
...
- **Run a file-integrity double-check before every commit** that modifies
  `Agents.md` or any large structured file. Confirm:
  1. `(Get-Content "Agents.md").Count` equals the pre-edit baseline plus the
     exact number of lines added (zero unexpected deletions).
  2. Every section heading that existed before the edit is still present:
     run `Select-String -Path "Agents.md" -Pattern "^### 9\."` and confirm
     the count has not decreased.
  3. All helper `.ps1` scripts used during the session have been removed with
     `remove_file` before staging for commit.
  If any check fails, restore from `git show HEAD:Agents.md` via a `.ps1`
  script and re-apply only the intended changes.
...
| Did you just `create_file` a `.ps1` that targets `Agents.md`, a `.resx` file, or `AppResources.Designer.cs`? | **STOP.** Call `get_file` on the `.ps1` before running it. Verify `[System.Text.Encoding]::UTF8` uses `::` (not `>::`), and that a `Write-Host` confirmation line is on the success path. Fix with `remove_file` + `create_file` if needed. |
