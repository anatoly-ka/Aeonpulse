...
| `Views/MainPage.xaml` | ~2244 | VibrantNature Taxonomy Flow chart - `VerticalStackLayout` x:Name=TaxonomyFlowContainer (IsVisible=VibrantNatureExpanded, Spacing=6): header Grid (TaxonomyDiscoveriesLabel col 0, TaxonomyExtinctionsLabel col 2); inner Grid (HeightRequest=160) with TaxonomyFlowView (GraphicsView, stream beziers only) + TaxonomyCircle (Border 68x68 CornerRadius=34, CardDark fill, Stroke=CyberCyan StrokeThickness=2) + Image icon_taxonomy.png (44x44, ImageTint.Color=CyberCyan, Centre/Centre). |
