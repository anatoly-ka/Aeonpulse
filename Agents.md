...
| `Models/FutharkRune.cs` | Edit freely | `DataTransferObject` | Immutable descriptor for one Elder Futhark rune. Carries `Symbol` (Unicode glyph), `Name`, `Brief`, `Full` (localised strings from AppResources via FutharkCatalogue.Build()), and `LineIndices` (int[] of 0-7 Web of Wyrd line indices). Used by `WyrdWebDrawable` and `ApplyWyrdWeb`. |
| `Models/FutharkCatalogue.cs` | Edit freely | `DataTransferObject` | Static builder for the 24-rune Elder Futhark catalogue. `Build()` returns fresh `IReadOnlyList<FutharkRune>` with AppResources strings; called each card-open so locale changes reflect. `IndexOf(catalogue, runeName)` finds rune by name. Line map: 0=top row, 1=mid row, 2=bot row, 3=left col, 4=center col, 5=right col, 6=diag TL-BR, 7=diag TR-BL. |
...
| File | Edit? | AIContext | Description |
|------|-------|-----------|-------------|
| `Views/WyrdWebDrawable.cs` | Edit freely | `UIPresentation` | `IDrawable` for the Web of Wyrd 3x3 matrix. Draws 8 skeleton lines (dim thin) then selected rune LineIndices as thick accent lines plus dot highlights. Colours: DefaultDark TextGray 25%/JubileeAccent; HC-Dark white; HC-Light black. Set on WyrdWebView.Drawable by ApplyWyrdWeb. |
...
| `Views/MainPage.xaml` | ~1565 | Birth Rune Web of Wyrd Explorer - `VerticalStackLayout` (`IsVisible`=BirthRuneExpanded): `GraphicsView` x:Name=WyrdWebView 200x200; `Label` x:Name=WyrdRuneName; `Label` x:Name=WyrdRuneMeaning; `FlexLayout` x:Name=WyrdRuneGrid (24 rune tap-targets built imperatively by `ApplyWyrdWeb`). |
